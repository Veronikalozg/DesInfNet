...
//b.Метод для установки стратегии хранения клиентов.
    public void saveAllClients() {
        strategy.saveAll(clients);
    }
//a.Метод для сохранения всех клиентов, используя стратегию сохранения.
    public List<Client> readAllClients() {
        return strategy.readAll();
    }
//Метод для чтения всех клиентов, используя стратегию чтения.
    public Client getById(int clientId) {
//c.Метод для получения клиента по его идентификатору (clientId).
...
//d.Метод для получения подсписка клиентов с пагинацией, где k – номер страницы, а n – количество клиентов на странице.
        int start = (k - 1) * n;
...
//e.Метод для сортировки списка клиентов по идентификатору.
...
//f.Метод для добавления нового клиента.
...
//g.Метод для замены информации о клиенте по его идентификатору.
...
//h.Метод для удаления клиента по его идентификатору.
...
//Использование стрима для фильтрации списка клиентов: оставляем только тех, у кого идентификатор не совпадает с удаляемым. Результат сохраняем обратно в список клиентов.
    public int getCount() {
//i.Метод для получения количества клиентов.
        return clients.size();
